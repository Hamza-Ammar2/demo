...
function startChat() {
  ...
  const steps = [
    ...
    {
      id: "consent",
      bot: "Share anonymized tracking logs with the research dataset? (optional)",
      choices: [
    ...
    s.choices.forEach((c) => {
      ...
      b.onclick = () => {
        ...
        if (i < steps.length) go();
          ...
          }).finally(() => runFull({ source: "intake", intake }));
        }
      };
...
async function runFull({ source, intake: answers = {} }) {
  ...
  try {
    ...
    // Phase model features derived from intake answers (Hamza fed upgrade)
