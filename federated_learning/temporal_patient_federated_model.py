...
PLOT_PATH = str(FL_DIR / "temporal_patient_comparison.png")
os.makedirs(OUTPUT_DIR, exist_ok=True)

# Hormone columns
HORM_COLS = ['lh', 'estrogen', 'pdg']

# 1. Models Definition (Temporal GRU Setup)
...
def main():
    ...
    for sub_id in unique_subject_ids:
        ...
        # Reshape to 2D to calculate mean/std, then shape back
    ...
    print(f"\nFiltered McPhases Cohort: Selected {num_clients} unique patient clients with sequential datasets.")
    if num_clients == 0:
    ...
    print(f"Running temporal pFL with rounds={rounds}")
    ...
    # =========================================================================
    print("\n" + "="*95)
    print("                      TEMPORAL PATIENT-CENTRIC SIMULATION RESULTS")
    ...
    plt.title(f'Temporal Patient-Centric Federated Learning Performance (Averaged across {num_clients} Clients)')
    ...
    plt.close()
    print(f"\nTemporal patient-centric simulation plot saved to: {PLOT_PATH}")
