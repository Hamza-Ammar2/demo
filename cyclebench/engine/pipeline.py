...
def _build_doctor_brief(case, findings, mode, missing_items) -> DoctorBrief:
    ...
    fallbacks = [
        "What other non-hormonal explanations should be investigated?",
        "Which labs or exams would be most informative next?",
        "How should symptom timing relative to the cycle change what we look for?",
        "Is anything in the history more urgent than a routine hormonal workup?",
    ]
    for q in fallbacks:
        if len(questions) >= 3:
            break
    ...
    # De-dupe while preserving order (padding / overlapping missing fields).
    deduped: list[str] = []
    for q in questions:
        if q not in deduped:
            deduped.append(q)
