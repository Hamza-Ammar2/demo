...
def train_hormonal_state(seed: int = 42) -> ModelBundle:
    ...
    metrics["data_source"] = "mcphases"
    metrics["data_redistributable"] = False
    metrics["disclaimer"] = (
        "Trained on PhysioNet mcPHASES under DUA. Raw participant rows are not in this "
        "repository; metrics and checkpoint may ship. Not a clinical diagnosis."
    )
    bundle = ModelBundle(
...
def train_menopause_stage(seed: int = 42, allow_synthetic: bool = True) -> ModelBundle:
    ...
    # Synthetic cohort is regenerable in-repo; real SWAN / ICPSR exports are not redistributed.
    metrics["data_redistributable"] = metrics["data_source"] == "synthetic_swan_like"
    metrics["split"] = "participant_held_out_25pct"
    metrics["disclaimer"] = (
        "Estimates menopausal *stage category* from hormones + symptoms + age. "
        "Not a clinical diagnosis of menopause onset timing. "
        "If data_source is synthetic_swan_like, accuracy is illustrative only."
    )
    bundle = ModelBundle(
